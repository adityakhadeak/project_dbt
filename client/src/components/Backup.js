...
const MergedData = () => {
    ...
    const readMultipleSheets = (file) => {
        return new Promise((resolve, reject) => {
            ...
            reader.onload = (e) => {
                ...
                console.log("College Data:", allData); // Debugging
                resolve(allData);
            };
            reader.onerror = (error) => reject(error);
            reader.readAsBinaryString(file);
        ...
        return new Promise((resolve, reject) => {
            ...
            reader.onload = (e) => {
                ...
                workbook.SheetNames.forEach(sheetName => {
                    const worksheet = workbook.Sheets[sheetName];
                    const jsonData = XLSX.utils.sheet_to_json(worksheet, { header: 1 });
                    console.log("DBT Data:", jsonData); // Debugging
                ...
                console.log("Combined DBT Data:", combinedData); // Debugging
                resolve(combinedData);
            };
            reader.onerror = (error) => reject(error);
            reader.readAsBinaryString(file);
    ...
    const mergeData = async (collegeData, dbtData) => {
        ...
        console.log("College Header:", collegeHeader); // Debugging
        console.log("College Rows:", collegeRows); // Debugging
        ...
        // Check merged data
        console.log("Merged Data:", merged); // Debugging
    
        const finalHeader = [
            'Name', 'Admission No', 'Batch', 'Year', 'Scheme', 'Application No', 'Disbursed Amount', 'Remaining Amount',
        ];
